...
//
// Vendored from heroiclabs/nakama-common (Apache-2.0). These declarations are shipped
// with the server rather than published to npm -- every package name you would guess
// 404s -- so the file lives here rather than in node_modules. Replaced by hand-written
// stubs at first, which typechecked perfectly and were wrong at runtime: the guessed
// `leaderboardRecordsList` signature hid the fact that it lists ONE owner's records
// rather than the players around them, and the ladder came back empty.
